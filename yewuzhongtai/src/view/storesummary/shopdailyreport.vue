    ...
    <div class="table-box" style="height: 40px;">
      ...
      >
        <el-option
          ...
          :label="item.shopName"
          :value="item.shopCode"
          :disabled="($store.state.userinfo.shopcode === 'accountant') || ($store.state.userinfo.shopcode === item.shopCode) ? undefined :'disabled'"
        ></el-option>
      </el-select>
...
export default {
  ...
  created(){
    console.log(this.$store.state.userinfo.shopcode , 'accountant');
  ...
  methods: {
    exportExcel() {
      require.ensure([], () => {
        ...
        const tHeader = ["门店名称", "日期", "客单价", "销量", "销售额"];
        // 上面设置Excel的表格第一行的标题
        const filterVal = ["shop_name", "order_date", "ta", "tc", "sales"];
    ...
    searchs() {
      ...
      _self.axios
        .post(_self.ApiUrl + "/home/DataReportD", formData1)
        .then(response => {
          if (response.data.errcode == "0") {
