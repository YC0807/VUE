  ...
  <div class="app-contion">
    ...
    <div id="tableid">
      <el-table :data="tableData" border style="width: 100%" :height="tableHeight">
        ...
        <el-table-column prop="bar_code" label="产品编码"></el-table-column>
        <el-table-column prop="amount_init" label="期初数"></el-table-column>
        <el-table-column prop="input" label="入库数"></el-table-column>
        <el-table-column prop="Sales" label="销售数"></el-table-column>
        <el-table-column prop="output" label="出库数"></el-table-column>
        <el-table-column prop="total_output" label="总出库数量"></el-table-column>
        <el-table-column prop="cost" label="单品成本"></el-table-column>
        <el-table-column prop="total_cost" label="应结金额"></el-table-column>
        <el-table-column prop="remains" label="结余数量"></el-table-column>
        <el-table-column prop="retn_amount" label="退款数"></el-table-column>
        <el-table-column prop="sales_amount" label="总入库数"></el-table-column>
      </el-table>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    searchs() {
      ...
      formData1.append("providerName", _self.provider);
      ...
      _self.axios
        .post(_self.ApiUrl + "/home/ProviderStoreInfo", formData1)
        .then(response => {
          ...
          if (response.data.errcode == "0") {
            ...
            for (let i = 0; i < data.length; i++) {
              ...
              item.production_name = data[i].productionName;
              item.amount_init = data[i].amount_init;
              item.input = data[i].amount_in;
              item.Sales = data[i].order_amount;
              item.output = data[i].amount_out;
              item.total_output = data[i].amount;
              item.cost = data[i].cost;
              item.total_cost = data[i].store_amount_pay;
              item.remains = data[i].store_amount;
    ...
    getdata() {
      let _self = this;
      _self.axios
        .post(_self.ApiUrl + "/home/ProviderInfoList")
        .then(response => {
          if (response.data.errcode == "0") {
            let list = [];
            let data = response.data.data.data;
            for (let i = 0; i < data.length; i++) {
              list.push(data[i].provider_name);
            }
            _self.options = list;
          }
        })
