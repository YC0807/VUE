...
<template>
  <div class="app-contion">
    <div class="table-box" style="height: 40px;">
      ...
      ></el-time-select>
      <el-checkbox v-model="isShowHours" label="小时" border></el-checkbox>
    ...
    <el-collapse v-model="activeNames" @change="handleChange">
      <el-collapse-item name="1">
        ...
        <!--<b>注意事项：</b>可选的时间是为了满足查询早餐或者某个点的销售情况，结束子时间是包括该小时的数据。<br>-->
        <b>报表功能：</b>查询某个时间区间的日环比，每行代表每日。
        <br />
        <b>使用方法：</b>选择必选时间只能查询一天的，选中日期双击即可。勾选小时可以查询小时数据
        <br />
        <b>使用案例：</b>查询2019年1月1日的报表情况。选择时间：(2019-01-01 到 2019-01-01 )
      </el-collapse-item>
    </el-collapse>
    ...
    <div id="tableid">
      <el-table :data="tableData" border style="width: 100%" :height="tableHeight">
        ...
        <el-table-column prop="order_date" label="小时" v-if="isShowHours"></el-table-column>
      </el-table>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    searchs() {
      ...
      if (_self.value2.length !== 0 && _self.value3.length !== 0) {
        ...
        formData1.append("startTime", _self.value1);
      ...
      if (this.isShowHours) {
        this.getDataByHour(formData1);
      } else {
        this.getDataByDay(formData1);
      }
    },
    // 每日
    getDataByDay(formData1) {
      let _self = this;
      _self.axios
        .post(_self.ApiUrl + "/home/intervalDataComparisonReportD", formData1)
        .then(response => {
