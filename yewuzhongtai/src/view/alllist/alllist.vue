<template>
  <div class="app-contion">
    ...
    >
      ...
      <el-table-column prop="rtn_pay" label="退款金额"></el-table-column>
      <el-table-column prop="rtn_amount" label="退款数量"></el-table-column>
      <el-table-column prop="address" label="传送小票" v-if="shopcode!='R_SAMCT'">
        <template slot-scope="scope">
          <el-button size="mini" @click="MySendData(scope.$index, scope.row)">传送</el-button>
        </template>
      </el-table-column>
                      ...
                      if (response.data.errcode == "0") {
                        ...
                        if (json.diningLocation == "2") {
                          LODOP.ADD_PRINT_TEXT(
                            ...
                            pageWidth,
                            rowHeight,
                            "地址：" + json.waybill.ship_address.substring(0,12)
                          );
                          hPos += rowHeight;
                          LODOP.ADD_PRINT_TEXT(
                            hPos,
                            ...
                            pageWidth,
                            rowHeight,
                            json.waybill.ship_address.substring(12)
                          );
                          hPos += rowHeight;
                          LODOP.ADD_PRINT_TEXT(
                            hPos,
