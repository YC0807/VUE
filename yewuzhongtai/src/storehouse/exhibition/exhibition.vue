  ...
  <div class="app-contion">
    ...
    <el-row>
      ...
      <el-table :data="tableData" style="width: 100%;margin-top: 0.625rem;" :height="tableHeight" border
        ...
        <el-table-column label="实际入库数">
          <template slot-scope="scope">
            <el-input v-model="scope.row.actual_amount" placeholder="请输入实际入库数" :disabled="tabledataDetailtab.ref_bill_type=='tranfer.out'||tabledataDetailtab.ref_bill_type=='warehouse.out'||tabledataDetailtab.ref_bill_type=='warehouse.return'||tabledataDetailtab.ref_bill_type=='warehouse.in'"
              @blur="actualamountchange(scope.$index, tableData)"></el-input>
          </template>
        </el-table-column>
        <el-table-column label="差额原因">
          <template slot-scope="scope">
            <el-input v-model="scope.row.margin_cause" placeholder="请输入差额原因" :disabled="tabledataDetailtab.ref_bill_type=='tranfer.out'||tabledataDetailtab.ref_bill_type=='warehouse.out'||tabledataDetailtab.ref_bill_type=='warehouse.return'||tabledataDetailtab.ref_bill_type=='warehouse.in'"
              @blur="actualamountchange(scope.$index, tableData)"></el-input>
          </template>
        </el-table-column>
      ...
      <div class=" f-s footer-increased" style="text-align: center;width: auto;padding: 0.625rem 0;" v-if="tabledataDetailtab.ref_bill_type=='tranfer.in'">
        <el-button type="primary" @click="goodsrejectedbtn" class='wids'>退货</el-button>
      </div>
    </el-row>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      goodsrejectedbtn() {
        ...
        // _self.axios.post(_self.ApiurlStorehouse + '/inventory/getTranfersWarehouseCodeInfo', formData, {
        //   // 单独配置
        ...
        //     let formData = new FormData();
        //     formData.append('warehouse_code', response.data.data.data.entryWarehouseCode); // 仓库编码
        //     formData.append('warehouse_name', response.data.data.data.entryWarehouseName); //仓库名称
        //     formData.append('type ', '6');
        //     formData.append('ref_order_code', response.data.data.data.ref_order_code);
        //     formData.append('ref_order_type', response.data.data.data.ref_order_type);
        ...
        //     //     _self.axios.post(_self.ApiurlStorehouse + '/inventory/updateReturnBills', {
        //     //       jsonData: listbox
        //     //     }, {
        //     //       // 单独配置
        //     //       withCredentials: false
      ...
      actualamountchange(index) {
        ...
        if (_self.tableData[index].actual_amount > _self.tableData[index].amount) {
          _self.$message({
        ...
        _self.Returngoods[index].amount = _self.tableData[index].amount - _self.tableData[index].actual_amount
        console.log(_self.Returngoods)
        let formData = new FormData();
        ...
        _self.axios.post(_self.ApiurlStorehouse + '/inventory/updateInvBillDetailActualamount', formData, {
          // 单独配置
          withCredentials: false
        }).then(response => {
      ...
      submitForm(formName) {
        ...
        _self.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            formData.append('supplierCode', '');
            _self.axios.post(_self.ApiurlStorehouse + '/inventory/updateInvBills', formData, {
              // 单独配置
              withCredentials: false
            }).then(response => {
