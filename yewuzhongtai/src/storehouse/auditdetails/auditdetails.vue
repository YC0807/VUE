...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      passthrough(index) {
        let _self = this;
        if (_self.tableData[index].ref_bill_type == 'warehouse.out' || _self.tableData[index].ref_bill_type ==
          'tranfer.out' || _self.tableData[index].ref_bill_type == 'warehouse.back') {
          let formData = new FormData();
                ...
                if (response.data.errcode == '0') {
                  if (_self.tableData[index].type == "调库") {
                    if (_self.tableData[index].ref_bill_type == 'tranfer.out') {
                      ...
                      formData.append('userId', _self.id);
                      _self.axios.post(_self.ApiurlStorehouse + '/inventory/inventoryApproval', formData, {
                        // 单独配置
                        withCredentials: false
                      }).then(response => {
